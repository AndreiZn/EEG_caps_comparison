...
EEG = eeg_checkset( EEG );
EEG = pop_editset(EEG, 'chanlocs', channel_location_file);
EEG = eeg_checkset( EEG );
eegplot('noui',EEG.data, 'winlength', size(y,2)/srate, 'eloc_file', channel_location_file);
fig = gcf;
chldrn = fig.Children; 
ax = chldrn(3);
xticks(ax, 1:srate*time_step_to_plot:size(y,2));
